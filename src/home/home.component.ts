...
export class HomeComponent implements OnInit, OnDestroy {
  ...
  private handleMagicLink(): void {
    ...
    console.log(
      'Processing magic link for email:',
      storedEmail || 'Not found - will wait for auth completion'
    );

    if (!storedEmail) {
      console.log(
        '⏳ No stored email, waiting for Firebase auth to complete...'
      );
      ...
      setTimeout(() => {
        this.authService.isLoggedIn$
          ...
          .subscribe((isAuthenticated) => {
            if (isAuthenticated) {
              console.log(
                '✅ Authentication completed via magic link, redirecting to dashboard'
              );
  ...
  openRoleSelectionModal(): void {
    ...
    this.authService.isLoggedIn$
      ...
      .subscribe((isAuthenticated: boolean) => {
        if (isAuthenticated) {
          console.log(
            '✅ User authenticated via modal check, redirecting to dashboard'
          );
          this.router.navigate(['/dashboard']);
        } else {
          console.log(
            '👤 User not authenticated, opening role selection modal'
          );
          this.roleModal.open();
        }
      });
